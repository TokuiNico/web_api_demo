...
def getFileSimilarity(name): # A file similarity (may be trajectories in range?)
    ...
    elif 'lon1' and 'lon2' and 'lat1' and 'lat2' and 'min_sup' and 'spatialThres' and 'temporalThres' in args:
        lon1 = args['lon1']
        lon2 = args['lon2']
        lat1 = args['lat1']
        lat2 = args['lat2']
        min_sup = args['min_sup']
        spatialThres = args['spatialThres']
        temporalThres = args['temporalThres']
        query = "SELECT * From cact.trajectory_artifical where lat>=" + lat1 + " and lat<=" + lat2 + " and lon>=" + lon1 + " and lon<=" + lon2 + ";"
...
#count km to kn total weights
    ...
    finalTrajList=[]
    for k in range(0, len(newKlist) , +1):

        for c in range(0, len(newKlist[k]), +1):
            RP = []
            PSi = []
            
            cindex = newKlist[k][c]# core set index
            ci = Clist[cindex]
                        ...
                        if(t!=BS):
                            ...
                            if(True):
                                for p in pointList:
                                    ...
                                    for index in range(0, len(PSi)-1 -1, +1):
                                        if(p.time<(PSi[index].time - temporalThres)):# this point over constraint
                                            break
                                        ...
                                        if(p.time>=(PSi[index].time - temporalThres) and p.time<=(PSi[index+1].time + temporalThres)):
                                            ...
                                            #30 m 
                                        if(mindistance<50 and p.time>=(PSi[ps].time - temporalThres) and p.time<=(PSi[pe].time + temporalThres)):
                                            if(EuclideanDistance(PSi[ps],p)!=0 and EuclideanDistance(p,PSi[pe])!=0):
                                                ...
                                                if(ps==0):
                                                    if(EuclideanDistance(PSi[pe],p)<EuclideanDistance(PSi[ps],PSi[pe])):
                                                    
                                                        PSi.insert(ps+1,new_p)
                                                elif(ps==len(PSi)-1):
                                                    if(EuclideanDistance(PSi[ps],p)<EuclideanDistance(PSi[ps],PSi[pe])):
                                                    
                                                        PSi.insert(ps+1,new_p)


                for index in range(0, len(PSi)-1, +1):
                    PSi[index].time = index
 ...
 #                 print 'Cluster: ', i
                ...
                RPi = sorted(RPi, key=attrgetter('time'))
            ...
            for index in range(0, len(RPi)-1, +1):
                print(str(RPi[index].time))
    ...
    return jsonify({"trajectories":jsonresult})
...
class DBSCAN:  
 #Density-Based Spatial Clustering of Application with Noise -> http://en.wikipedia.org/wiki/DBSCAN  
   def __init__(self):  
     self.name = 'DBSCAN'  
     self.DB = [] #Database  
     self.esp = 3 #neighborhood distance for search  
